...
et's take a look at where all those properties are. 
...
#Looking for properties close to our data that might sawy the data


We are going to create a dataset of properties that are close to key officials.

Here we are looking at the Gillispies, the people alleged to have taken the property, two people who bought a lot of land bank properties, and the land bank itself
...
Lawrence_key <- Lawrence_Geo %>% 
  filter(str_detect(OWNER,"GILLISPIE"))  #|str_detect("LAMBIOTTE")|("FINLEY"))  #Mike Finley
...
model = Predictor$new(learner, data = test, y = "Land_Bank")
x.interest = data.frame(test[1, ])
